...
CAPABILITY_BUDGET_OVERRIDES: dict[str, tuple[int, int, int]] = {
    ...
    "define_requirements": (1900, 6, 7600),
    "propose_agent_design": (2200, 7, 9800),
    "critique_agent_design": (1800, 6, 7600),
    "synthesize_blueprint_narrative": (2200, 6, 9200),
    "recommend_minimal_tools": (2100, 7, 9200),
    "analyze_estimation_risks": (2200, 7, 9600),
}
            ...
            refs.append(
                ApprovedArtifactReference(
                    ...
                    summary=_compact_text(
                        ...
                        f"Memoria: {snapshot.blueprint.memory_strategy}. "
                    ...
                    source_refs=["session.blueprint", "session.blueprint_versions"],
                    stage_affinity=["design", "tools", "memory", "validate", "estimate", "package"],
                    agent_affinity=["builder", "planner", "retrieval", "memory"],
                )
            )
...
class StageKnowledgePlanner:
    ...
    def _build_query(
        ...
        *,
        ...
        parts.extend(item.summary for item in approved_refs[:3])
